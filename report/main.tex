...
Algorithm~\ref{alg:retrieve} gives the full procedure. The forward-adjacency map $A[\cdot]$ is precomputed once from the heterogeneous \texttt{edge\_index} tensors ($O(|\mathcal{E}|)$ time, $O(|\mathcal{E}|)$ space), so each query thereafter costs $O(\sum_m b^{L_m})$ where $b$ is mean branching and $L_m$ is the length of meta-path $m$. In practice for $L{\le}3$ and $b{\approx}30$ this is fast enough to run on CPU.

\begin{algorithm}[t]
\small
\caption{\textsc{RetrieveTopK}: cosine-scored meta-path subgraph retrieval}
\label{alg:retrieve}
\begin{algorithmic}[1]
\Require query pair $(c,d)$; heterogeneous graph $\mathcal{G}$; embeddings $\{\mathbf{h}_v\}$; meta-path set $\mathcal{M}$; budget $K$
\Ensure ranked list of top-$K$ paths
\State $A\gets \textsc{BuildForwardAdjacency}(\mathcal{G})$ \Comment{precomputed once}
\State $\mathcal{P}\gets [\,]$
\ForAll{$m\in\mathcal{M}$ with edge-type sequence $(e_1,\ldots,e_L)$}
  \State $F\gets\{(c,)\}$ \Comment{BFS frontier of partial paths}
  \For{$\ell=1,\ldots,L$}
    \State $F\gets\{\,(v_0,\ldots,v_\ell)\;:\;(v_0,\ldots,v_{\ell-1})\in F,\;v_\ell\in A[e_\ell][v_{\ell-1}]\,\}$
  \EndFor
  \ForAll{$p=(v_0,\ldots,v_L)\in F$ with $v_L=d$}
    \State $s(p)\gets \frac{1}{L}\sum_{i=0}^{L-1}\cos(\mathbf{h}_{v_i},\mathbf{h}_{v_{i+1}})$
    \State append $(p,s(p),m)$ to $\mathcal{P}$
  \EndFor
\EndFor
\State sort $\mathcal{P}$ by $s(p)$ descending
\State \Return first $K$ entries of $\mathcal{P}$
\end{algorithmic}
\end{algorithm}
...
% ---- Figure 2: Main results bar chart (generated by figures/make_figures.py)
\begin{figure}[t]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_bars.pdf}
...
% ---- Figure 3: Top-K ablation (generated by figures/make_figures.py)
\begin{figure}[t]
\centering
...
Drug repurposing is widely regarded as the cheapest and fastest path to new therapies for neglected and rare diseases; a reliable GraphRAG system that both predicts and \emph{justifies} candidate compounds could meaningfully shorten this path, particularly for conditions where prospective trials are prohibitive. Our pipeline is fully open-source and uses a public benchmark (Hetionet), removing one barrier to entry for small academic groups or low-resource settings. Positive externalities therefore include democratised access to interpretable drug-repurposing tooling and a transparent baseline against which future closed-loop systems can be compared.

We are, however, explicit about three risks. First, our $52.3\%$ faithfulness rate (\S\ref{sec:exp:faith}) means that almost half of the natural-language rationales contain entities or mechanistic claims that the retrieved subgraph does not support; \emph{any} downstream use that confuses a plausible explanation with a verified one is unsafe. Second, Hetionet is a snapshot, not ground truth: biases in the underlying databases (e.g., the over-representation of well-studied proteins) propagate into predictions and can amplify existing research inequities across diseases. Third, LLM-based reasoning is non-deterministic and subject to training-data drift; reported accuracies may not be reproducible as the hosted Grok model is updated. For all of these reasons, the pipeline is intended as a \emph{hypothesis-generation} tool, not a clinical recommender, and any translational use must be accompanied by expert curation and wet-lab validation.

% ====================================================================
\section{Reproducibility}
\label{sec:reproducibility}

All experiments use fixed random seeds (GNN split seed $42$; training seed $42$; bootstrap seed $0$). Hetionet is downloaded directly from the official GitHub release at load time (SHA-pinned via a caching wrapper). Hyperparameters are documented in \S\ref{sec:method:gnn} and \S\ref{sec:exp} and duplicated in \texttt{scripts/train\_gnn.py} / \texttt{scripts/train\_kge.py} argparse defaults. The full $N{=}150$ per-case JSONL (including every LLM rationale and judge decision) and the \texttt{ablation\_k.json} output are shipped in the \texttt{runs/} directory of the public repository, so the main-results and error-analysis tables can be regenerated offline without an API key by running \texttt{experiments/error\_analysis.py} and \texttt{experiments/recompute\_kge.py}. Confidence intervals are from $2{,}000$ bootstrap resamples. The only non-deterministic component is the Grok LLM; to mitigate this we set \texttt{temperature}\,$=0$, but we acknowledge that hosted-model updates may shift future reproductions by several percentage points.

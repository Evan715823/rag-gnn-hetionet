...
% ---------------- geometry (NeurIPS-like) ----------------
\usepackage[letterpaper,
            textwidth=5.5in, textheight=9in,
            top=1in, left=1.5in]{geometry}
...
\fancyfoot[L]{\footnotesize COMP 559 \textbullet{} Machine Learning with Graphs \textbullet{} Spring 2026}
\fancyfoot[R]{\thepage}

% ---------------- colors ----------------
\usepackage{xcolor}
\definecolor{accent}{HTML}{1F4E79}
\definecolor{clrInput}{HTML}{F5A962}   % orange
\definecolor{clrGNN}{HTML}{4E8ABF}     % blue
\definecolor{clrRetr}{HTML}{58B36E}    % green
\definecolor{clrVerb}{HTML}{E6C463}    % yellow
\definecolor{clrLLM}{HTML}{8E6FB0}     % purple
\definecolor{clrOut}{HTML}{C0504D}     % red
...
% ---------------- hyperref ----------------
\usepackage[colorlinks=true,linkcolor=black,citecolor=accent,urlcolor=accent]{hyperref}
...
\newcommand{\ctd}{\textsc{C--t--D}\xspace}

% ---------------- custom title block (NeurIPS-like) ----------------
\makeatletter
\renewcommand{\maketitle}{%
  \begin{center}%
    {\LARGE\bfseries \@title\par}\vspace{4pt}%
    {\rule{0.9\textwidth}{0.5pt}}\vspace{8pt}%
    {\large \@author\par}\vspace{4pt}%
    {\small\itshape \@date\par}\vspace{6pt}%
...
\author{%
  \begin{tabular}{c@{\hspace{2em}}c@{\hspace{2em}}c}
    Jingwen Chen & Jingtao Wang & Shouyu Wang \\
    Rice University & Rice University & Rice University \\
    \texttt{jc380@rice.edu} & \texttt{jw306@rice.edu} & \texttt{sw188@rice.edu}
  \end{tabular}%
}

\date{COMP 559 --- Machine Learning with Graphs --- Spring 2026 Final Project}
...
% ---- Figure 1: Pipeline diagram with colour-coded stages and drop shadows
\begin{figure}[t]
\centering
\begin{tikzpicture}[
    ...
    stage/.style={
        ...
        top color=#1!30, bottom color=#1!55,
        minimum height=13mm, minimum width=32mm,
        align=center, font=\small,
        drop shadow={shadow xshift=1.2pt,shadow yshift=-1.2pt,opacity=0.25,fill=black}
    },
    flow/.style={-{Stealth[length=2mm]}, line width=1.1pt, draw=black!55},
    edgelbl/.style={font=\scriptsize\sffamily,fill=white,inner sep=1pt}
]
\node[stage=clrInput] (q)               {\textbf{Query}\\[-1pt]\footnotesize $(c,d)\in\mathcal{C}{\times}\mathcal{D}$};
\node[stage=clrGNN, right=of q] (gnn)   {\textbf{Hetero.\ GNN}\\[-1pt]\footnotesize SAGEConv$\,\circ\,$\texttt{to\_hetero}};
\node[stage=clrRetr, right=of gnn] (retr){\textbf{Retrieval}\\[-1pt]\footnotesize top-$K$ meta-paths};
\node[stage=clrVerb, below=of retr] (verb){\textbf{Verbaliser}\\[-1pt]\footnotesize template-based};
\node[stage=clrLLM, left=of verb] (llm) {\textbf{LLM reasoner}\\[-1pt]\footnotesize \texttt{grok-4-fast-reasoning}};
\node[stage=clrOut, left=of llm] (out)  {\textbf{Output $+$ judge}\\[-1pt]\footnotesize $\{$pred, conf, rationale$\}$};

\draw[flow] (q)    -- (gnn);
\draw[flow] (gnn)  -- (retr)   node[edgelbl,midway,above] {$\{\mathbf{h}_v\}$};
\draw[flow] (retr) -- (verb)   node[edgelbl,midway,right=1pt] {paths};
\draw[flow] (verb) -- (llm)    node[edgelbl,midway,above] {prompt};
\draw[flow] (llm)  -- (out)    node[edgelbl,midway,above] {JSON};
\end{tikzpicture}
...
Following \citet{himmelstein2017}, we define nine biologically-motivated meta-paths (Table~\ref{tab:metapaths}). For a query $(c,d)$, we enumerate every instantiation of each template using precomputed forward adjacency dictionaries, giving a candidate set $\mathcal{P}(c,d)$. Each path $p=(v_0,v_1,\ldots,v_L)$ with $v_0{=}c,\,v_L{=}d$ is scored by the mean cosine similarity between consecutive node embeddings:
\begin{equation}
s(p) = \frac{1}{L}\sum_{i=0}^{L-1} \cos\!\bigl(\mathbf{h}_{v_i},\,\mathbf{h}_{v_{i+1}}\bigr).
\label{eq:pathscore}
\end{equation}
The top-$K$ scoring paths are forwarded to the verbaliser. Eq.~\eqref{eq:pathscore} is the open-loop analogue of a learned retriever; \S\ref{sec:fut} discusses closing the loop.

\begin{table}[t]
\centering
\small
\caption{Nine meta-path templates used for subgraph retrieval. Abbreviations follow Hetionet convention (e.g.\ \texttt{b}=binds, \texttt{i}=interacts, \texttt{a}=associates, \texttt{u}=upregulates, \texttt{d}=downregulates, \texttt{r}=resembles, \texttt{p}=palliates, \texttt{t}=treats).}
\label{tab:metapaths}
\begin{tabular}{@{}lll@{}}
\toprule
...
CuGuD    & Compound-up-Gene-up-Disease                                      & co-upregulation \\
CdGdD    & Compound-down-Gene-down-Disease                                  & co-downregulation \\
CuGdD    & Compound-up-Gene-down-Disease                                    & compensatory axis \\
CdGuD    & Compound-down-Gene-up-Disease                                    & compensatory axis \\
...
CbGpPpG  & Compound-binds-Gene-participates-Pathway-participates-Gene       & pathway proxy \\
\bottomrule
\end{tabular}
\end{table}
...
% ---- Figure 2: Per-method coloured bar chart
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
    width=\linewidth, height=0.34\textwidth,
    ybar, ymin=0, ymax=1.02,
    ylabel={Accuracy}, ylabel style={font=\small}, ytick={0,0.25,0.5,0.75,1.0},
    ymajorgrids, grid style={dashed,gray!30},
    symbolic x coords={{LLM-only},{GNN-only},{KGE (calib.)},{GNN-RAG+LLM (ours)}},
    xtick=data, x tick label style={font=\small},
    bar width=38pt, enlarge x limits=0.17,
    axis lines*=left,
    every axis plot/.append style={
        error bars/.cd, y dir=both, y explicit,
        error bar style={line width=0.9pt,black!60}
    },
    nodes near coords, nodes near coords style={font=\scriptsize,/pgf/number format/.cd,fixed,precision=3},
]

\addplot[ybar,fill=cLLM,draw=cLLM!60!black,bar width=38pt] coordinates {({LLM-only},0.553)    +- (0.080,0.080)};
\addplot[ybar,fill=cGNN,draw=cGNN!60!black,bar width=38pt] coordinates {({GNN-only},0.727)    +- (0.074,0.066)};
\addplot[ybar,fill=cKGE,draw=cKGE!60!black,bar width=38pt] coordinates {({KGE (calib.)},0.787)+- (0.067,0.066)};
\addplot[ybar,fill=cOurs,draw=cOurs!70!black,bar width=38pt,
         postaction={pattern=north east lines,pattern color=white}]
  coordinates {({GNN-RAG+LLM (ours)},0.873) +- (0.060,0.047)};

\draw[dashed,red!60!black,line width=0.8pt] (axis cs:{LLM-only},0.5) -- (axis cs:{GNN-RAG+LLM (ours)},0.5);
\node[red!60!black,font=\scriptsize,anchor=east] at (axis cs:{LLM-only},0.5) {chance\ \ };
\end{axis}
\end{tikzpicture}
\caption{Accuracy and $95\%$ bootstrap CIs on the $N{=}150$ evaluation set. \ours (hatched, dark blue) strictly dominates every baseline, and all pairwise differences against it are significant under McNemar's test (Table~\ref{tab:main}). Red dashed line marks random-guess accuracy.}
\label{fig:bars}
\end{figure}
...
We vary the number of paths handed to the LLM from $K{=}0$ (LLM-only) to $K{=}10$, holding all other components fixed. The result (Figure~\ref{fig:ablation}) exhibits a striking non-monotonic shape: accuracy rises by $23$ percentage points from $K{=}0$ to $K{=}1$, saturates at $K{=}5$, and \emph{declines} at $K{=}10$.

% ---- Figure 3: Line chart with area fill and peak marker
\begin{figure}[t]
\centering
\begin{tikzpicture}
\begin{axis}[
    width=\linewidth, height=0.34\textwidth,
    xlabel={Number of retrieved paths $K$},
    ylabel={Accuracy},
    ymin=0.45, ymax=0.96,
    xmin=-0.5, xmax=10.5,
    xtick={0,1,3,5,10},
    ytick={0.5,0.6,0.7,0.8,0.9},
    ymajorgrids, grid style={dashed,gray!25},
    axis lines*=left,
    label style={font=\small}, tick label style={font=\small},
]

% area under the curve
\addplot[draw=none,fill=accent!15,domain=0:10,forget plot]
  coordinates {(0,0.525) (1,0.750) (3,0.800) (5,0.875) (10,0.825)}
  \closedcycle;

% main line
\addplot[color=accent,line width=1.4pt,mark=*,mark size=2.8pt,
         mark options={fill=accent!80,draw=accent!60!black,line width=0.6pt}]
  coordinates {
    (0,0.525) (1,0.750) (3,0.800) (5,0.875) (10,0.825)
  };

% peak marker
\addplot[color=red!80!black,mark=star,mark size=7pt,only marks,
         mark options={line width=1.2pt}]
  coordinates {(5,0.875)};

% callouts
\node[anchor=north,font=\scriptsize,text=accent]  at (axis cs:0,0.525) {0.525};
\node[anchor=north,font=\scriptsize,text=accent]  at (axis cs:1,0.750) {0.750};
\node[anchor=north,font=\scriptsize,text=accent]  at (axis cs:3,0.800) {0.800};
\node[anchor=south,font=\scriptsize\bfseries,text=red!80!black] at (axis cs:5,0.895) {$K^\star{=}5\;\; 0.875$};
\node[anchor=north,font=\scriptsize,text=accent]  at (axis cs:10,0.825) {0.825};

% drop annotation
\draw[->,red!70!black,thick,>=Stealth]
  (axis cs:5.8,0.875) to[bend left=25] (axis cs:9.5,0.840);
\node[red!70!black,font=\scriptsize\itshape] at (axis cs:7.7,0.92) {$-5$\,pts drop};
\end{axis}
\end{tikzpicture}
\caption{Top-$K$ retrieval ablation ($N{=}40$ per configuration, $20{+}/20{-}$). Accuracy saturates at $K^\star{=}5$ (red star) and drops five points at $K{=}10$. We attribute the decline to attention dilution: redundant or weakly scored paths crowd out the most informative chain.}
\label{fig:ablation}
\end{figure}
...
\centering
\small
\caption{Error-analysis buckets over the $N{=}150$ test set.}
\label{tab:buckets}
\begin{tabular}{@{}lr@{\hspace{3em}}lr@{}}
\toprule
Bucket & Count & Bucket & Count \\
\midrule
\gnno \& \ours both right          & 95 & retrieval helped (\ours over \llmo) & \textbf{61} \\
\gnno wrong, \ours right (rescue)  & 36 & retrieval hurt   (\ours over \llmo) & 13 \\
\gnno right, \ours wrong           & 14 & ratio                                 & $4.7\times$ \\
Both wrong                         & 5  & & \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Small topological signal, large interpretive leverage.} Using the information-theoretic decomposition of \citet{bertschinger2014unique}, \citet{hays2026raggnn} estimate that retrieval contributes only $8.6\%$ of the unique predictive information above topology. Our absolute-accuracy gain of $+15$ points from \gnno to \ours (Table~\ref{tab:main}) at first sight seems to contradict this estimate. The tension dissolves once we note that LLM reasoning is not adding \emph{new} information so much as reorganising the GNN's own embedding distances into an explanatory chain that the binary classifier discards. The information-theoretic and task-utility views are therefore complementary, not competing.
...
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Hand-designed meta-paths.} Our nine templates are a biologist's shortlist rather than an exhaustive search of Hetionet's meta-path space. Automatic meta-path mining along the lines of PathCon~\citep{wang2021pathcon} would reduce this bias.

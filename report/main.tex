...
\usepackage{mathptmx}          % Times-compatible Roman
...
% ---------- title ----------
\title{\vspace{-1.5em}\Large GNN-Guided Meta-Path Retrieval for LLM Reasoning\\in Drug Repurposing on Hetionet}

\author{%
  Jingwen Chen \\ Rice University \\ \texttt{jc380@rice.edu}
  \and
  Jingtao Wang \\ Rice University \\ \texttt{jw306@rice.edu}
  \and
  Shouyu Wang \\ Rice University \\ \texttt{sw188@rice.edu}
}

\date{\small COMP 559 --- Machine Learning with Graphs --- Spring 2026}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw, rounded corners=2pt, minimum height=11mm, minimum width=30mm, align=center, fill=gray!6, font=\small},
    arrow/.style={-Stealth, thick, draw=black!60}
]
\node[box] (q)    {Query\\$(c,d)\in\mathcal{C}\times\mathcal{D}$};
\node[box, right=of q]   (gnn)  {Hetero. GNN\\\footnotesize SAGEConv $\circ$ to\_hetero};
\node[box, right=of gnn] (retr) {Meta-path\\retrieval (top-$K$)};
\node[box, below=of retr] (verb) {Path verbaliser\\\footnotesize template-based};
\node[box, left=of verb] (llm)  {LLM reasoner\\\footnotesize grok-4-fast-reasoning};
\node[box, left=of llm]  (out)  {$\{$pred, conf,\\rationale$\}$ + judge};

\draw[arrow] (q)    -- (gnn);
\draw[arrow] (gnn)  -- (retr) node[midway, above, font=\scriptsize] {$\{\mathbf{h}_v\}$};
\draw[arrow] (retr) -- (verb) node[midway, right=1pt, font=\scriptsize] {paths};
\draw[arrow] (verb) -- (llm)  node[midway, above, font=\scriptsize] {prompt};
\draw[arrow] (llm)  -- (out)  node[midway, above, font=\scriptsize] {JSON};
\end{tikzpicture}
\caption{End-to-end pipeline. The heterogeneous GNN provides node embeddings used to score candidate meta-path instances; top-$K$ paths are verbalised and handed to a reasoning LLM, whose rationale is separately audited by a judge.}
\label{fig:pipeline}
\end{figure}
...
\begin{equation}
s(p) = \frac{1}{L}\sum_{i=0}^{L-1} \cos(\mathbf{h}_{v_i},\mathbf{h}_{v_{i+1}}).
\label{eq:pathscore}
\end{equation}
...
\begin{tabular}{@{}lll@{}}
\toprule
ID & Template & Biological interpretation \\ \midrule
...
\begin{tabular}{@{}lrlr@{}}
\toprule
Node type & Count & Relation (undirected)          & Count \\ \midrule
...
\begin{tabular}{@{}lccc@{}}
\toprule
Method & Accuracy & $95\%$ CI & vs \ours \\ \midrule
...
\ours                           & \textbf{0.873} & \textbf{[0.813, 0.920]} & --- \\ \bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}
\begin{axis}[
    width=0.80\textwidth, height=0.32\textwidth,
    ybar, ymin=0, ymax=1.02,
    ylabel={Accuracy}, ylabel style={font=\small}, ytick={0,0.25,0.5,0.75,1.0},
    symbolic x coords={{LLM-only},{GNN-only},{KGE (calib.)},{GNN-RAG+LLM}},
    xtick=data, x tick label style={font=\small},
    bar width=34pt, enlarge x limits=0.15,
    nodes near coords, nodes near coords style={font=\scriptsize, /pgf/number format/.cd, fixed, precision=3},
    error bars/.cd, y dir=both, y explicit,
]
\addplot+[ybar, fill=blue!25, draw=blue!60!black] coordinates {
    ({LLM-only},    0.553) +- (0.080, 0.080)
    ({GNN-only},    0.727) +- (0.074, 0.066)
    ({KGE (calib.)},0.787) +- (0.067, 0.066)
    ({GNN-RAG+LLM}, 0.873) +- (0.060, 0.047)
};
\end{axis}
\end{tikzpicture}
\caption{Accuracy and $95\%$ bootstrap CIs on the $N{=}150$ evaluation set. \ours strictly dominates every baseline, and all pairwise differences against it are significant under McNemar's test (Table~\ref{tab:main}).}
\label{fig:bars}
\end{figure}
...
\begin{tikzpicture}
\begin{axis}[
    width=0.70\textwidth, height=0.34\textwidth,
    ...
    mark size=2.5pt, thick,
    label style={font=\small}, tick label style={font=\small},
]
\addplot[color=blue!70!black, mark=*, line width=1.2pt] coordinates {
    (0,  0.525)
    (1,  0.750)
    (3,  0.800)
    (5,  0.875)
...
\node[anchor=south, font=\scriptsize] at (axis cs:0,0.525) {0.525};
\node[anchor=south, font=\scriptsize] at (axis cs:1,0.750) {0.750};
\node[anchor=south, font=\scriptsize] at (axis cs:3,0.800) {0.800};
\node[anchor=south, font=\scriptsize] at (axis cs:5,0.875) {\textbf{0.875}};
\node[anchor=south, font=\scriptsize] at (axis cs:10,0.825) {0.825};
\draw[dashed, red!70, thick] (axis cs:5,0.45) -- (axis cs:5,0.92) node[above, font=\scriptsize, red!70!black] {optimal $K^\star$};
\end{axis}
\end{tikzpicture}
\caption{Top-$K$ retrieval ablation ($N{=}40$ per configuration, $20{+}/20{-}$). Accuracy saturates at $K^\star{=}5$ and drops $5$ points at $K{=}10$. We attribute the decline to attention dilution: redundant or weakly scored paths crowd out the most informative chain.}
\label{fig:ablation}
\end{figure}
...
\toprule
Bucket & Count & Bucket & Count \\ \midrule
...
Both wrong                         & 5  & & \\ \bottomrule
\end{tabular}
\end{table}

The most informative row is the \emph{rescue} bucket: $36$ out of $41$ cases where the GNN was wrong were corrected by LLM reasoning over the retrieved paths. A recurrent rescue pattern is the $\text{CrCtD}$ meta-path (analogue rescue): if an approved compound $c'$ structurally resembling $c$ is known to treat $d$, the LLM promotes the prediction. Conversely, the $14$ \emph{regressions} ($c$-correct-by-GNN, $c$-wrong-by-\ours) concentrate on high-GNN-score pairs where a single weak retrieved path (often a single-hop $\text{CbGaD}$ with $s(p){<}0$) mislead the LLM into an over-confident positive prediction. The ratio of \texttt{retrieval\_helped}:\texttt{retrieval\_hurt}$\,=\,61{:}13\,=\,4.7{\times}$ is a tidy summary of net benefit---and a useful fairness figure for future work that compares alternative retrievers.
...
All code, model checkpoints recipes, per-case predictions and bootstrap scripts are at \url{https://github.com/Evan715823/rag-gnn-hetionet}.
